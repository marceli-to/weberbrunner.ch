...
<template>
	<Grid :cols="10">
		...
		<Span class="col-span-8">
			<div class="bg-white font-semibold min-h-30 border border-black flex justify-between items-center px-20 select-none" :class="{ 'opacity-50': !publish }">
				<span>{{ label }}</span>
				<span class="flex gap-x-20">
					<Pencil class="w-14 cursor-pointer" @click="$emit('edit')" />
					<Eye :variant="publish ? 'visible' : 'hidden'" class="w-14 cursor-pointer" @click="$emit('toggle-publish')" />
				</span>
			</div>
		</Span>
