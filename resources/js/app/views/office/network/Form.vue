...
onMounted(async () => {
	if (isEdit.value) {
		const { data } = await networkApi.show(route.params.id)
...
async function handleSubmit() {
	let ok
	if (isEdit.value) {
		ok = await submit(() => networkApi.update(route.params.id, { text: form.value.text }))
	} else {
		ok = await submit(() => networkApi.store({ text: form.value.text, section_id: sectionId.value }))
	}
	if (ok) {
		router.push({ name: 'office.network' })
	}
}
	...
	<FormContainer @submit="handleSubmit">
		...
		<Grid class="mb-40">
			...
			<Span class="col-span-8">
				<PageTitle>Netzwerk / {{ sectionTitle }}</PageTitle>
			</Span>
		</Grid>
