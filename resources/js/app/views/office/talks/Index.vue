...
const groups = ref([])
const { collapsed, toggle: toggleSection } = useCollapsed('talks')
const title = ref('')
...
async function fetchEntries() {
	const { data } = await talksApi.index()
	groups.value = data.data
}

async function storeSection() {
	const ok = await submit(() => sectionsApi.store({ title: title.value, type: 'talk' }))
	if (ok) {
		close()
		await fetchEntries()
...
async function deleteEntry(entry) {
	...
	if (ok) {
		await talksApi.destroy(entry.uuid)
		await fetchEntries()
	}
}
...
async function togglePublish(entry) {
	...
	await talksApi.toggle(entry.uuid)
}
                ...
                @click="toggleSection(group.section.uuid)">
						...
						<Span v-show="!collapsed.has(group.section.uuid)" class="col-span-10 col-start-1">
										...
										<Span class="col-span-8">
											<div class="bg-white font-semibold min-h-30 border border-black flex justify-between items-center px-20 select-none" :class="{ 'opacity-50': !entry.publish }">
												...
												<span class="flex gap-x-20">
													<Pencil class="w-14 cursor-pointer" @click="router.push({ name: 'talks.edit', params: { id: entry.uuid } })" />
							...
							<Grid :cols="10" class="mb-10">
								<Span class="col-span-8 col-start-2">
									<Button class="px-20" @click="router.push({ name: 'talks.create', query: { section: group.section.uuid } })">
										<template #icon-right>
											<Plus class="w-10 h-10" />
										</template>
										Neuer Eintrag
