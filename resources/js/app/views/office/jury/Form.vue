...
onMounted(async () => {
	if (isEdit.value) {
		const { data } = await juryApi.show(route.params.id)
...
async function handleSubmit() {
	let ok
	if (isEdit.value) {
		ok = await submit(() => juryApi.update(route.params.id, { text: form.value.text }))
	} else {
		ok = await submit(() => juryApi.store({ text: form.value.text, section_id: sectionId.value }))
	}
	if (ok) {
		router.push({ name: 'office.jury' })
	}
}
	...
	<FormContainer @submit="handleSubmit">
		...
		<Grid class="mb-40">
			...
			<Span class="col-span-8">
				<PageTitle>Jury / {{ sectionTitle }}</PageTitle>
			</Span>
		</Grid>
