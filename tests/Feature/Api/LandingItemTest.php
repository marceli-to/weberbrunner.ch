...
it('requires authentication', function () {
	$this->getJson('/api/dashboard/homepage')->assertUnauthorized();
});
...
it('lists landing items grouped by column', function () {
	...
	$this->getJson('/api/dashboard/homepage')
		->assertOk()
...
it('stores a landing item', function () {
	...
	$this->postJson('/api/dashboard/homepage', [
...
it('computes sort_order per column', function () {
	...
	$this->postJson('/api/dashboard/homepage', ['project_id' => $p1->id, 'column' => 1]);
	$this->postJson('/api/dashboard/homepage', ['project_id' => $p2->id, 'column' => 1]);
	$this->postJson('/api/dashboard/homepage', ['project_id' => $p3->id, 'column' => 2]);
...
it('prevents duplicate project placement', function () {
	...
	$this->postJson('/api/dashboard/homepage', ['project_id' => $project->id, 'column' => 1])
		->assertCreated();

	$this->postJson('/api/dashboard/homepage', ['project_id' => $project->id, 'column' => 2])
		->assertUnprocessable()
...
it('validates column must be 1, 2 or 3', function () {
	...
	$this->postJson('/api/dashboard/homepage', ['project_id' => $project->id, 'column' => 4])
		->assertUnprocessable()
...
it('deletes a landing item', function () {
	...
	$this->deleteJson("/api/dashboard/homepage/{$item->uuid}")
		->assertNoContent();
...
it('reorders landing items within and across columns', function () {
	...
	$this->patchJson('/api/dashboard/homepage/reorder', [
		'items' => [
